...
public class CapabilityStatementCustomizer {
	...
	public void customize(IBaseConformance theCapabilityStatement) {
		...
		if (config.getOauth().getEnabled()) {
			...
			cs.getRestFirstRep().setSecurity(getSecurityComponent());
		}
	}
	...
	private CapabilityStatementRestSecurityComponent getSecurityComponent() {
		...
		extensions.add(new Extension("authorize", new UriType(config.getOauth().getAuthorize_url())));
		extensions.add(new Extension("token", new UriType(config.getOauth().getToken_url())));
		extensions.add(new Extension("manage", new UriType(config.getOauth().getManage_url())));

		List<Extension> extensionsList = new ArrayList<>();
		extensionsList.add((Extension)
				new Extension(new UriType("http://fhir-registry.smarthealthit.org/StructureDefinition/oauth-uris"))
