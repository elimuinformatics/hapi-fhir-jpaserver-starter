...
public class WellknownEndpointHelper {
	...
	/**
	 * Helper function to return metadata. See
	 * https://www.hl7.org/fhir/smart-app-launch/conformance.html#using-well-known.
	 *
	...
	private static JSONArray getResponseTypesSupported() {
		return new JSONArray().put("code").put("token");
	}

	private static JSONArray getCapabilities() {
		return new JSONArray().put("sso-openid-connect").put("context-patient").put("context-tenant");
	}

	private static JSONArray getCodeChallengeMethodsSupported() {
		return new JSONArray().put("S256");
	}
}
