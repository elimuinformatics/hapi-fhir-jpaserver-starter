...
public class CustomConsentService implements IConsentService {
	...
	public ConsentOutcome canSeeResource(
		...
		if (!isUsingOAuth(theRequestDetails) || !isTaskRequest(theRequestDetails)) {
	...
	private IBaseResource getPersistentResource(RequestDetails theRequestDetails) {
		...
		try {
			return daoRegistry.getResourceDao(resourceName).read(theRequestDetails.getId(), theRequestDetails);
		} catch (RuntimeException e) {
			if (logger.isDebugEnabled()) {
				logger.error(
						"Unexpected error getting persistent resource type '{}': {}", resourceName, e.getMessage());
			}
			throw e;
		}
	}
	...
	private boolean isJsonPatchRequestPropertyValid(DocumentContext json, String propertyName, String patientId) {
		...
		Filter filter = filter(where("path").is(pathValue).and("value").ne("Patient/" + patientId));
